...
Frontend del proyecto realizado en Next.js para consultar atracciones, mapas y preguntas frecuentes de IRTRA Petapa, consumiendo datos desde PocketBase.
...
- **games**: `name`, `image`, `map_img`, `time` (minutos), `tags`/`tag` (array/string), `out_color`, `inner_color`, `description`, `rules_height` (array de `{ min, max, condicion }`), `restrictions`, `closed` (bool).  
- **walkthrough**: `title`, `description`, `image` opcional.  
