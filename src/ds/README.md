# PetapaOnTrack DS
Módulo de análisis de datos y explicación de backend del proyecto: pipelines de limpieza, notebooks de modelado y scripts que empujan resultados en vivo hacia PocketBase.
...
2. **Variables de entorno (`.env`)**
   ...
   PB_URL=https://<tu-pocketbase-host>
   PB_ADMIN_EMAIL=...
   PB_ADMIN_PASSWORD=...
   ```
...
- **Actualizar elementos** (modelos y CSV) via `scp`:
  ```bash
...
- **Logs rápidos**: cada script imprime un dict con timestamp + input + predicción `tail -f`.